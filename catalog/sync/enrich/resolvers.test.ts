...
describe('each field resolves independently', () => {
  test('context falls through to the canonical index when the feed is silent', () => {
    const r = resolveContext({ spec: null, intrinsic: null, canonical: CANON })!;
  ...
  test('max output resolves separately from context', () => {
    ...
    const r = resolveMaxOutput({ intrinsic: null, spec: { contextTokens: 256_000 }, canonical: CANON })!;
...
describe('cost semantics keep effective and reference apart', () => {
  test('a per-token provider reports its own price as effective', () => {
    const c = resolveCost({ intrinsic: null, spec: { costInPerM: 10, costOutPerM: 50 }, canonical: CANON }, 'per_token');
  ...
  test('a zero price is free, and that is a real answer', () => {
    const c = resolveCost({ intrinsic: null, spec: { costInPerM: 0, costOutPerM: 0 }, canonical: CANON }, 'per_token');
    assert.equal(c.kind, 'free');
  });

  test('a subscription provider with no per-token price reports Included, not unknown', () => {
    const c = resolveCost({ spec: null, intrinsic: null, canonical: CANON }, 'subscription');
  ...
  test('a per-token provider that published nothing is unknown, not free', () => {
    const c = resolveCost({ spec: null, intrinsic: null, canonical: null }, 'per_token');
...
describe('the billing declaration is a fallback, never an override', () => {
  test("a subscription provider's own published price still wins", () => {
    ...
    const c = resolveCost({ intrinsic: null, spec: { costInPerM: 0.14, costOutPerM: 0.28 }, canonical: CANON }, 'subscription');
    assert.equal(c.kind, 'per_token');
    assert.equal(c.inPerM, 0.14);
  });

  test('it only decides what a MISSING price means', () => {
    const sub = resolveCost({ spec: null, intrinsic: null, canonical: CANON }, 'subscription');
    const pay = resolveCost({ spec: null, intrinsic: null, canonical: CANON }, 'per_token');
...
describe('cross-provider facts: intrinsic only', () => {
  const POOL = { structured: true, tools: true, declaredBy: 'nano-gpt/qwen/qwen3.5-plus' };
  ...
  test('price is never taken from another provider either', () => {
    const c = resolveCost({ spec: null, intrinsic: POOL, canonical: null }, 'per_token');
    assert.equal(c.kind, 'unknown');
    assert.equal(c.inPerM, null);
  });

  test('the pool can say no as well as yes — it is a declaration, not a parameter list', () => {
    const r = resolveCapability('structured', { spec: null, intrinsic: { structured: false, declaredBy: 'x/y' }, canonical: null })!;
    assert.equal(r.value, false);
  });
});
