...
import { Alert, Button, FormField, Input, Stepper } from "@venom/design-system/primitives";
    ...
    <div className="flex min-h-screen items-center justify-center bg-surface-canvas p-4">
      <div className="flex w-full max-w-sm flex-col gap-4">
        <div className="text-center">
          ...
          <p className="vn-caption">First run — create the single owner password. There are no users, roles, or teams.</p>
        </div>
        ...
        <form className="vn-card vn-card--pad flex flex-col gap-3" onSubmit={handleSubmit} noValidate>
          ...
          {validationError ? <TypedErrorDisplay tone="warning" code="validation_error" message={validationError} /> : null}
          ...
          <Alert tone="info" title="No recovery email exists.">
            If you lose this password, recovery is a backup restore or the documented local reset. Consider creating an
            encrypted backup once set up.
          </Alert>

          <Button type="submit" variant="primary" icon="shield" loading={submitting} disabled={submitting || locked}>
            Create password &amp; continue
          </Button>
        </form>

        <div className="text-center">
          <OwnerSessionStatus state={locked ? "locked_out" : "unauthenticated"} retryAfter={locked ? formatRetryAfter(remainingLockout) : undefined} />
        </div>
      </div>
    </div>
  );
